#!/usr/bin/env python3
"""
Process Apollo 13 Flight Journal HTML files and convert them to JSON format.
...
def extract_content_from_html(html_path):
    ...
    if not transcript:
        ...
        for bold in soup.find_all('b'):
            ...
            # Check if this looks like a timestamp (e.g., "109:15:45")
            ts_match = re.match(r'^(' + timestamp_re + r')\b', bold_text)
            if not ts_match:
                continue
            timestamp = ts_match.group(1)

            # Some LSJ variants embed the speaker in the same <b>:
            #   "003:13:50 Roosa:"  or  "003:21:21 Scott (onboard):"
...
def process_journal_files():
    ...
    exclude_files = ['ap13fj_index.html', 'flight_journal_links.txt']
